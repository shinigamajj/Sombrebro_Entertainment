...
ds_map_replace(startNode,"estimatedCostToGoal",CStar_getEstimatedCost(startNode,goalNode));
...
while (!ds_priority_empty(openList)) {
    ...
    if (node == goalNode) {
        ...
        return CStar_constructPath(goalNode,argument0,argument1,argument4);
    }
    neighbors = CStar_getNeighbors(node);
    for (i=0; i<ds_list_size(neighbors); i+=1) {
        ...
        costFromStart = ds_map_find_value(node,"costFromStart") + CStar_getCost(node,neighborNode);
        if ((!isOpen && !isClosed) || (costFromStart < ds_map_find_value(neighborNode,"costFromStart"))) {
            ...
            ds_map_replace(neighborNode,"estimatedCostToGoal",
                           CStar_getEstimatedCost(neighborNode,goalNode));
            if (isClosed)
